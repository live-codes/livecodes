...
`;var _=`<ul id="settings-menu" class="dropdown-menu">\r
  <li>\r
...
`;var M=`<section data-lang="art-template">\r
...
`;var C=`<div id="test-editor-container" class="modal-container">
...
`;var H=`<div id="import-container" class="modal-container">
...
`;var U=`<div id="deploy-container" class="modal-container">
...
`;var R=`<div id="sync-container" class="modal-container">
...
`;var A=`<div id="broadcast-container" class="modal-container">
...
`;var B=`<div id="welcome-container" class="modal-container">\r
...
`;var P=`<div id="about-container" class="modal-container">\r
...
`;var D=`<div id="info-container" class="modal-container">
...
`;var I=`<div id="resources-container" class="modal-container">
...
`;var O=`<div id="login-screen" class="modal-container">\r
...
`;var J=`<div id="prompt-recover-screen">
...
`;var F=`<div id="templates-container" class="modal-container">
...
`;var $=`<div id="list-container" class="list-container">
...
`;var X=`<div id="add-asset-container" class="modal-container">
...
`;var W=`<div id="snippets-list-container" class="list-container">
...
`;var z=`<div id="add-snippet-container" class="modal-container">
...
`;var Y=`<div id="share-screen" class="modal-container">\r
...
`;var V=`<div id="embed-container" class="modal-container">
...
`;var K=`<div id="editor-settings-container" class="modal-container">
...
`;var a=e=>e.replace(/{{APP_VERSION}}/g,"4").replace(/{{SDK_VERSION}}/g,"0.0.2").replace(/{{COMMIT_SHA}}/g,"fe430fb").replace(/{{REPO_URL}}/g,"https://github.com/live-codes/livecodes").replace(/{{DOCS_BASE_URL}}/g,"/livecodes/docs/"),ta=a(E),aa=a(T),oa=a(_),ra=a(M),na=a(j),sa=a(C),ia=a(H),la=a(U),Z=a(R),ca=a(q),pa=a(A),da=a(B),ma=a(P),ua=a(D),ga=a(I),ha=a(O),ba=a(N),fa=a(J),va=a(F),ya=a($),wa=a(G),xa=a(X),ka=a(W),Sa=a(z),La=a(Y),Ea=a(V),Ta=a(K),_a=a(Q);var te=["jspm","skypack"],ae=["unpkg","jsdelivr"],oe=["jsdelivr.gh","statically"],h={getModuleUrl:(e,{isModule:o=!0,defaultCDN:r="jspm"}={})=>{e=e.replace(/#nobundle/g,"");let t=ee(e,o,r);return t||(o?"https://jspm.dev/"+e:"https://cdn.jsdelivr.net/npm/"+e)},getUrl:(e,o)=>e.startsWith("http")?e:ee(e,!1,o||ze())||e,cdnLists:{npm:ae,module:te,gh:oe},checkCDNs:async(e,o)=>{let r=[o,...h.cdnLists.npm].filter(Boolean);for(let t of r)try{if((await fetch(h.getUrl(e,t),{method:"HEAD"})).ok)return t}catch{}return h.cdnLists.npm[0]}},ze=()=>{if(globalThis.appCDN)return globalThis.appCDN;try{return new URL(location.href).searchParams.get("appCDN")||h.cdnLists.npm[0]}catch{return h.cdnLists.npm[0]}},ee=(e,o,r)=>{let t=o&&e.startsWith("unpkg:")?"?module":"";e.startsWith("gh:")?e=e.replace("gh",oe[0]):e.includes(":")||(e=(r||(o?te[0]:ae[0]))+":"+e);for(let n of Ye){let[i,l]=n;if(i.test(e))return e.replace(i,l)+t}return null},Ye=[[/^(jspm:)(.+)/i,"https://jspm.dev/$2"],[/^(npm:)(.+)/i,"https://jspm.dev/$2"],[/^(node:)(.+)/i,"https://jspm.dev/$2"],[/^(skypack:)(.+)/i,"https://cdn.skypack.dev/$2"],[/^(jsdelivr:)(.+)/i,"https://cdn.jsdelivr.net/npm/$2"],[/^(jsdelivr.gh:)(.+)/i,"https://cdn.jsdelivr.net/gh/$2"],[/^(statically:)(.+)/i,"https://cdn.statically.io/gh/$2"],[/^(esm.run:)(.+)/i,"https://esm.run/$2"],[/^(esm.sh:)(.+)/i,"https://esm.sh/$2"],[/^(esbuild:)(.+)/i,"https://esbuild.vercel.app/$2"],[/^(bundle.run:)(.+)/i,"https://bundle.run/$2"],[/^(unpkg:)(.+)/i,"https://unpkg.com/$2"],[/^(bundlejs:)(.+)/i,"https://deno.bundlejs.com/?file&q=$2"],[/^(bundle:)(.+)/i,"https://deno.bundlejs.com/?file&q=$2"],[/^(deno:)(.+)/i,"https://deno.bundlejs.com/?file&q=https://deno.land/x/$2/mod.ts"],[/^(https:\/\/deno\.land\/.+)/i,"https://deno.bundlejs.com/?file&q=$1"],[/^(github:|https:\/\/github\.com\/)(.[^\/]+?)\/(.[^\/]+?)\/(?!releases\/)(?:(?:blob|raw)\/)?(.+?\/.+)/i,"https://deno.bundlejs.com/?file&q=https://cdn.jsdelivr.net/gh/$2/$3@$4"],[/^(gist\.github:)(.+?\/[0-9a-f]+\/raw\/(?:[0-9a-f]+\/)?.+)$/i,"https://gist.githack.com/$2"],[/^(gitlab:|https:\/\/gitlab\.com\/)([^\/]+.*\/[^\/]+)\/(?:raw|blob)\/(.+?)(?:\?.*)?$/i,"https://deno.bundlejs.com/?file&q=https://gl.githack.com/$2/raw/$3"],[/^(bitbucket:|https:\/\/bitbucket\.org\/)([^\/]+\/[^\/]+)\/(?:raw|src)\/(.+?)(?:\?.*)?$/i,"https://deno.bundlejs.com/?file&q=https://bb.githack.com/$2/raw/$3"],[/^(bitbucket:)snippets\/([^\/]+\/[^\/]+)\/revisions\/([^\/\#\?]+)(?:\?[^#]*)?(?:\#file-(.+?))$/i,"https://bb.githack.com/!api/2.0/snippets/$2/$3/files/$4"],[/^(bitbucket:)snippets\/([^\/]+\/[^\/\#\?]+)(?:\?[^#]*)?(?:\#file-(.+?))$/i,"https://bb.githack.com/!api/2.0/snippets/$2/HEAD/files/$3"],[/^(bitbucket:)\!api\/2.0\/snippets\/([^\/]+\/[^\/]+\/[^\/]+)\/files\/(.+?)(?:\?.*)?$/i,"https://bb.githack.com/!api/2.0/snippets/$2/files/$3"],[/^(api\.bitbucket:)2.0\/snippets\/([^\/]+\/[^\/]+\/[^\/]+)\/files\/(.+?)(?:\?.*)?$/i,"https://bb.githack.com/!api/2.0/snippets/$2/files/$3"],[/^(rawgit:)(.+?\/[0-9a-f]+\/raw\/(?:[0-9a-f]+\/)?.+)$/i,"https://gist.githack.com/$2"],[/^(rawgit:|https:\/\/raw\.githubusercontent\.com)(\/[^\/]+\/[^\/]+|[0-9A-Za-z-]+\/[0-9a-f]+\/raw)\/(.+)/i,"https://deno.bundlejs.com/?file&q=https://raw.githack.com/$2/$3"]];var{getUrl:re,getModuleUrl:Ha}=h;var ne=re("@tarekraafat/autocomplete.js@10.2.6/dist/autoComplete.js");var Ve="0.6.64",Ua=re(`malinajs@${Ve}/malina.js`);var Ke=(e,o)=>({Accept:`application/vnd.github.v3${o?"."+o:""}+json`,"Content-Type":"application/json",Authorization:"token "+e.token});var se=async(e,o="public")=>{let r=1,t=100,n=5,i=[];for(;r<=n;){let l=await fetch(`https://api.github.com/user/repos?type=${o}&per_page=${t}&page=${r}`,{method:"GET",cache:"no-store",headers:Ke(e)});if(r+=1,!l.ok)continue;let s=await l.json();i.push(...s.map(d=>d.name)),s.length<t&&(r=n+1)}return i};var y=()=>document.querySelector("#sync-link"),ie=()=>document.querySelector("#sync-indicator");var le=e=>e.querySelector("#new-repo-form");var ce=e=>e.querySelector("#new-repo-name");var pe=e=>e.querySelector("#new-repo-autosync"),de=e=>e.querySelector("#existing-repo-form");var w=e=>e.querySelector("#existing-repo-name");var me=e=>e.querySelector("#existing-repo-autosync");var ue=e=>(e||document).querySelector("#sync-status"),ge=e=>(e||document).querySelectorAll(".start-sync-btn");var Qe=(e,o)=>{let r=document.createElement("div");r.innerHTML=Z;let t=r.firstChild,n=t.querySelectorAll("#sync-tabs li");return n.forEach(i=>{e.addEventListener(i,"click",()=>{n.forEach(s=>s.classList.remove("active")),i.classList.add("active"),document.querySelectorAll("#sync-screens > div").forEach(s=>{s.classList.remove("active")});let l=t.querySelector("#"+i.dataset.target);l?.classList.add("active"),l?.querySelector("input")?.focus()})}),o&&setTimeout(()=>{n[1].click();let i=w(t);i.value=o}),t},x="Sync in progress...",v=()=>y()?.dataset.hint===x,b=({inProgress:e,lastSync:o,syncContainer:r})=>{let t=y(),n=ie(),i=ue(r),l=ge(r),s=o?`Last sync: ${new Date(o).toLocaleString()}`:"";i&&(i.innerText=s),e??v()?(t&&(t.classList.add("hint--bottom"),t.dataset.hint=x,n?.classList.remove("hidden")),l?.forEach(d=>{d.innerText=x,d.disabled=!0})):(t&&(t.classList.toggle("hint--bottom",!!s),t.dataset.hint=s,n?.classList.add("hidden")),l?.forEach(d=>{d.innerText="Sync",d.disabled=!1}))},Fa=async({baseUrl:e,modal:o,notifications:r,eventsManager:t,user:n,deps:i})=>{let l=await i.getSyncData(),s=Qe(t,l?.repo),d=le(s),k=ce(s),he=pe(s),be=de(s),f=w(s),fe=me(s);b({inProgress:v(),lastSync:l?.lastSync,syncContainer:s});let ve=import(e+"sync.0516f201dac9fd021aeba54ab85f6049.js").then(p=>(p.init(e),p)),S=(p,c,m)=>(r.info("Sync started..."),o.close(),ve.then(async g=>{if(!await g.sync({user:p,repo:c,newRepo:m})){r.error("Sync failed!");return}r.success("Sync complete!")}).catch(()=>{r.error("Sync failed!")}));t.addEventListener(d,"submit",async p=>{if(p.preventDefault(),!n||v())return;let c=k.value,m=he.checked,g=!0;if(!c){r.error("Repo name is required");return}b({inProgress:!0}),await S(n,c,g);let u=Date.now();await i.setSyncData({autosync:m,repo:c,lastSync:u}),b({inProgress:!1,lastSync:u})}),t.addEventListener(be,"submit",async p=>{if(p.preventDefault(),!n||v())return;let c=f.value,m=fe.checked,g=!1;if(!c){r.error("Repo name is required");return}b({inProgress:!0}),await S(n,c,g);let u=Date.now();await i.setSyncData({autosync:m,repo:c,lastSync:u}),b({inProgress:!1,lastSync:u})});let L;import(ne).then(async()=>{if(L=globalThis.autoComplete,!n)return;let p=await se(n,"all");t.addEventListener(f,"init",()=>{f.focus()});let c="#"+f.id;if(!document.querySelector(c))return;let m=new L({selector:c,placeHolder:"Search your repos...",data:{src:p},resultItem:{highlight:{render:!0}}});t.addEventListener(m.input,"selection",function(g){let u=g.detail;m.input.blur();let ye=u.selection.value;m.input.value=ye})}),o.show(s,{isAsync:!0}),k.focus()};export{Fa as createSyncUI,v as isSyncInProgress,b as updateSyncStatus};
