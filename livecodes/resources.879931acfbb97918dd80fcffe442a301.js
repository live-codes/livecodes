...
`;var D=`<ul id="settings-menu" class="dropdown-menu">\r
  <li>\r
...
`;var O=`<section data-lang="art-template">\r
...
`;var J=`<div id="custom-settings-container" class="modal-container">
...
`;var N=`<div id="import-container" class="modal-container">
...
`;var W=`<div id="sync-container" class="modal-container">
...
`;var G=`<div id="backup-container" class="modal-container">
...
`;var z=`<div id="broadcast-container" class="modal-container">
...
`;var Y=`<div id="welcome-container" class="modal-container">\r
...
`;var $=`<div id="info-container" class="modal-container">
...
`;var K=`<div id="resources-container" class="modal-container">
...
`;var Q=`<div id="login-screen" class="modal-container">\r
...
`;var ee=`<div id="prompt-recover-screen">
...
`;var te=`<div id="templates-container" class="modal-container">
...
`;var ae=`<div id="list-container" class="list-container">
...
`;var oe=`<div id="assets-list-container" class="list-container">
...
`;var ne=`<div id="add-asset-container" class="modal-container">
...
`;var re=`<div id="snippets-list-container" class="list-container">
...
`;var ie=`<div id="add-snippet-container" class="modal-container">
...
`;var le=`<div id="share-screen" class="modal-container">\r
...
`;var se=`<div id="embed-container" class="modal-container">
...
`;var ce=`<div id="editor-settings-container" class="modal-container">
...
`;var i=o=>o.replace(/{{APP_VERSION}}/g,"4").replace(/{{SDK_VERSION}}/g,"0.0.2").replace(/{{COMMIT_SHA}}/g,"fe430fb").replace(/{{REPO_URL}}/g,"https://github.com/live-codes/livecodes").replace(/{{DOCS_BASE_URL}}/g,"/livecodes/docs/"),oa=i(U),na=i(B),ra=i(D),ia=i(O),la=i(J),sa=i(F),ca=i(N),da=i(X),pa=i(W),ua=i(G),ma=i(z),ga=i(Y),ha=i(V),ba=i($),pe=i(K),fa=i(Q),va=i(Z),ya=i(ee),wa=i(te),ka=i(ae),Sa=i(oe),La=i(ne),xa=i(re),Ea=i(ie),Ta=i(le),_a=i(se),Ma=i(ce),Ha=i(de);var ue=o=>`https://ofcncog2cu-dsn.algolia.net/1/indexes/npm-search/${encodeURIComponent(o)}?x-algolia-agent=Browser`,me={"X-Algolia-Application-Id":"OFCNCOG2CU","X-Algolia-API-Key":"f54e21fa3a2a0160595bb058179bfb1e"},ge=["name","description","homepage","repository.url","version"],A="https://data.jsdelivr.com/v1",q={"User-Agent":"https://livecodes.io"},L=o=>{let s=o.startsWith("@"),c=s?o.slice(1):o,[t,d]=c.split("@");return[(s?"@":"")+t,d]},$e=async(o,s=10)=>{let c={page:0,hitsPerPage:s,attributesToHighlight:[],attributesToRetrieve:ge,analyticsTags:["jsdelivr"]},[t,d]=L(o),e;if(d){let l=await x(o);typeof l=="string"&&(e=L(l)[1])}let a=await fetch(ue("query"),{method:"POST",headers:{"Content-Type":"application/json; charset=UTF-8",...me},body:JSON.stringify({query:t,...c})}).then(l=>{if(!l.ok)throw new Error("failed to fetch");return l}).then(l=>l.json()).catch(l=>({error:!0,message:l.mesage||String(l)}));if("error"in a)return a;let f=a.hits.map(l=>(l.name===t&&e&&(l.version=e),l.repository?.url&&(l.repo=l.repository?.url),l));return f.sort((l,v)=>l.name===t?-1:v.name===t?1:0),f},x=async o=>{let s=`${A}/package/resolve/npm/${o}`,c=await fetch(s,{headers:q}).then(e=>{if(!e.ok)throw new Error("failed to fetch");return e}).then(e=>e.json()).catch(e=>({error:!0,message:e.mesage||String(e)}));if("error"in c)return c;let t=L(o)[0],d=c.version;return d?`${t}@${d}`:t},Ke=async o=>{let[s,c]=L(o),t;if(c){let a=await x(o);typeof a=="string"&&(t=L(a)[1])}let d=ue(s)+"&attributesToRetrieve="+ge.join(","),e=await fetch(d,{method:"GET",headers:me}).then(a=>{if(!a.ok)throw new Error("failed to fetch");return a}).then(a=>a.json()).catch(a=>({error:!0,message:a.mesage||String(a)}));return"error"in e||(t&&(e.version=t),e.repository?.url&&(e.repo=e.repository?.url)),e},Qe=async o=>{let s=await x(o),c=`${A}/package/npm/${s}/flat`,t=await fetch(c,{headers:q}).then(e=>{if(!e.ok)throw new Error("failed to fetch");return e}).then(e=>e.json()).catch(e=>({error:!0,message:e.mesage||String(e)}));if("error"in t)return t;let d=`https://cdn.jsdelivr.net/npm/${s}`;return{...t.default?{default:d+t.default}:{},files:t.files.map(e=>d+e.name)}},Ze=async o=>{let s=await x(o),c=`${A}/package/npm/${s}/entrypoints`,t=await fetch(c,{headers:q}).then(e=>{if(!e.ok)throw new Error("failed to fetch");return e}).then(e=>e.json()).catch(e=>({error:!0,message:e.mesage||String(e)}));if("error"in t)return t;let d=`https://cdn.jsdelivr.net/npm/${s}`;return{...t.js?.file?{js:d+t.js?.file}:{},...t.css?.file?{css:d+t.css?.file}:{}}},P={search:$e,getPkgInfo:Ke,getPkgFiles:Qe,getPkgDefaultFiles:Ze};var he=(o,s)=>{let c;return(...t)=>{c&&clearTimeout(c),c=setTimeout(()=>o.apply(null,t),typeof s=="function"?s():s)}};var be=o=>{let s=()=>{o.style.display="none",e(),window.watchingEscape=!1},c=a=>{!o.contains(a.target)&&d(o)&&s()},t=a=>{a.key==="Escape"&&(s(),a.preventDefault())},d=a=>!!a&&!!(a.offsetWidth||a.offsetHeight||a.getClientRects().length),e=()=>{document.removeEventListener("click",c),document.removeEventListener("keydown",t)};return document.addEventListener("click",c),document.addEventListener("keydown",t),window.watchingEscape=!0,{clear:()=>e()}};var fe=()=>document.querySelectorAll("#resources-container textarea"),ve=()=>document.querySelectorAll('#resources-container input[type="radio"]'),ye=()=>document.querySelector("#resources-container #resources-load-btn");var Ba=({baseUrl:o,modal:s,eventsManager:c,deps:t})=>{let d=document.createElement("div");d.innerHTML=pe;let e=d.firstChild;s.show(e);let a=fe();a.forEach(n=>{let p=t.getConfig()[n.dataset.resource];n.value=p.length!==0?p.join(`
`)+`
`:""});let f=ve();f.forEach(n=>{t.getConfig().cssPreset===n.value&&(n.checked=!0)});let l=document.querySelector("#resources-container #resources-search-input"),v=document.querySelector("#resources-container #resources-result-container"),g=document.querySelector("#resources-container #resources-search-results"),y=document.querySelector("#resources-container #fonts-container select"),E=document.querySelector("#resources-container #fonts-container button"),T=(n,p)=>{if(!n||_(n,p))return;let m=p==="stylesheets"?a[0]:a[1];m.value=m.value.trim()===""?n.trim()+`
`:m.value.split(`
`).map(r=>r.trim()).filter(r=>r!=="").join(`
`)+`
`+n.trim()+`
`,document.querySelector(`#resources-search-results a.add-resource[data-resource-url="${n}"]`)?.classList.add("resource-added")},_=(n,p)=>n?!!(p==="stylesheets"?a[0]:a[1]).value.split(`
`).map(r=>r.trim()).find(r=>r===n.trim()):!1,we=({name:n,version:p,description:m,files:r})=>{let b=document.createElement("li"),h=document.createElement("div");h.classList.add("search-result-item-title"),h.textContent=n,b.appendChild(h);let M=document.createElement("span");M.classList.add("label-description"),M.textContent="v"+p,h.appendChild(M);let H=document.createElement("div");H.classList.add("search-result-item-description"),H.textContent=m||"",b.appendChild(H);let w=document.createElement("div");w.classList.add("search-result-item-files"),b.appendChild(w);let C=document.createElement("span");if(C.classList.add("search-result-item-script"),w.appendChild(C),r?.js){let S=_(r.js,"scripts"),u=document.createElement("a");u.classList.add("add-resource"),u.classList.toggle("resource-added",S),u.href="#",u.onclick=S?null:()=>T(r.js||"","scripts"),u.title=r.js,u.dataset.resourceUrl=r.js,u.innerHTML=`<img src="${o}assets/images/javascript.svg" />`,C.appendChild(u)}let R=document.createElement("span");if(R.classList.add("search-result-item-stylesheet"),w.appendChild(R),r?.css){let S=_(r.css,"stylesheets"),u=document.createElement("a");u.classList.add("add-resource"),u.classList.toggle("resource-added",S),u.href="#",u.onclick=S?null:()=>T(r.css||"","stylesheets"),u.title=r.css,u.dataset.resourceUrl=r.css,u.innerHTML=`<img src="${o}assets/images/css.svg" />`,R.appendChild(u)}let j=document.createElement("span");j.classList.add("search-result-item-Browse"),w.appendChild(j);let k=document.createElement("a");return k.href=`https://cdn.jsdelivr.net/npm/${n}/`,k.target="_blank",k.title="Browse package files on jsDelivr",k.textContent="Browse",j.appendChild(k),b},ke=async()=>{let n=l.value,p=await P.search(l.value,5);if(n!==l.value){g.innerHTML='<li><div class="search-result-item-description">Loading...</div></li>';return}if("error"in p){g.innerHTML='<li><div class="search-result-item-error">Failed to load results!</div></li>';return}if(p.length===0){g.innerHTML=`<li><div class="search-result-item-description">No results found for: <strong>${n}</strong></div></li>`;return}let m=(await Promise.all(p.map(async r=>{let b=r.version!=null?`${r.name}@${r.version}`:r.name,h=await P.getPkgDefaultFiles(b);if(!("error"in h))return{...r,files:h}}))).filter(Boolean);if(n===l.value){if(m.length===0){g.innerHTML='<li><div class="search-result-item-error">Failed to load results!</div></li>';return}g.innerHTML="",m.forEach(r=>g.appendChild(we(r)))}},I;l?.focus(),l.addEventListener("input",async()=>{if(g.innerHTML='<li><div class="search-result-item-description">Loading...</div></li>',I?.clear(),l.value.length>0)v.style.display="unset",I=be(v);else{v.style.display="none";return}he(ke,300)()}),import(o+"google-fonts.5685f46a9bcf414b4147377d6e78852b.js").then(n=>{y.innerHTML='<option value="">Select font ...</option>',n.googleFonts.getFonts().forEach(m=>{let r=document.createElement("option");r.innerText=m,y.appendChild(r)}),c.addEventListener(E,"click",()=>{y.value!==""&&(T(n.googleFonts.getStylesheetUrl(y.value),"stylesheets"),y.value="",E.innerText="\u2714",setTimeout(()=>{E.innerText="Add"},1e3))})}),c.addEventListener(ye(),"click",async()=>{a.forEach(n=>{let p=n.dataset.resource;t.setConfig({...t.getConfig(),[p]:n.value?.split(`
`).map(m=>m.trim()).filter(m=>m!=="")||[]})}),f.forEach(n=>{n.checked&&t.setConfig({...t.getConfig(),cssPreset:n.value})}),t.loadResources()})};export{Ba as createExternalResourcesUI};
