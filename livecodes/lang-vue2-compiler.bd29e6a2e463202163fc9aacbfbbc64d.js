...
`:"",d=await j(p+s,i.name,e),m=g(d);s=m.code,o={...o,...m.info}}return{code:s,info:o}},N=async(t,e,r,s={})=>{let o=await O(t,e);typeof s.prepareFn=="function"&&(o=await s.prepareFn(o,r));let n=r.processors.filter(p=>!s.skipCompilers?.includes(p)).length>0,c=((p,d="lang")=>`(<${p}\\s*)(?:([\\s\\S]*?)${d}\\s*=\\s*["']([A-Za-z0-9 _]*)["'])?((?:[^>]*)>)([\\s\\S]*?)(<\\/${p}>)`)(e,s.languageAttribute),i=[];for(let p of[...o.matchAll(new RegExp(c,"g"))]){let[d,m,h="",x="",v,y,A]=p;if((!x||!y)&&(e!=="style"||!n)){i.push(d);continue}let f=C(x);if((!f||s.skipCompilers?.includes(f))&&(e!=="style"||!n)){i.push(d);continue}let b="";["typescript","jsx","tsx","babel","sucrase"].includes(f||"")&&(b=D(y));let u=(await j(y+b,f,r)).code||y;b&&(u=u.replace(b,"")),n&&(u=g(await _(u,r,f??"css")).code),i.push(d.replace(new RegExp(c,"g"),e==="template"&&s.removeEnclosingTemplate?u:m+h+v+u+A))}return o.replace(new RegExp(c,"g"),()=>i.pop()||"")},E=async(t,e,r={})=>{let s=["template","style","script"];for(let o of s)t=await N(t,o,e,r);return t};var M=t=>`const options = {
  moduleCache: {
    vue: Vue,
  },
