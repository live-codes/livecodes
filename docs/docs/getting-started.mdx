...
It is packed with [features](./features/index.mdx) and offers various ways to [import code](./features/import.mdx).

## Embedded Playgrounds

LiveCodes playgrounds can be easily [embedded](./features/embeds.mdx) into any web page. This can be achieved using:

### App Embed Screen

The [standalone app](#standalone-app) allows you to embed any project from the [embed screen](./features/embeds.mdx). The UI allows setting embed options and shows a preview of the embedded playground.
...
LiveCodes [<abbr title="Software Development Kit">SDK</abbr>](./sdk/index.mdx) is available as [npm package](https://www.npmjs.com/package/livecodes) to allow easy embedding and communication with playgrounds.
...
You may use any of the methods to [prefill the playground](./features/code-prefill.mdx) with your own code.
...
Please refer to the section on [Embedded Playgrounds](./features/embeds.mdx) for more details.
...
Please check the section on [self-hosting](./features/self-hosting.mdx) for other methods of self-hosting, including the built-in setup to deploy to GitHub pages and how to use the SDK with the self-hosted app.
