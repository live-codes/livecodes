...
Please note that LiveCodes supports running [JSX](./jsx.mdx) and [TSX](./tsx.mdx) which are compiled to JavaScript using the [TypeScript compiler](./typescript.mdx). This was the traditional way of running React and other JSX/TSX apps in LiveCodes before React compiler support was added.

## Usage

For usage and examples, see documentation for [React](./react.mdx) and [TypeScript](./typescript.mdx) support in LiveCodes.
...
- `react-tsx`: the option `disableAutoRender` can be set to `true` to disable [auto-rendering](./react.mdx#auto-rendering).
