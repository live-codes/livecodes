...
TSX is also supported in LiveCodes and is [documented here](./tsx.mdx).

Please note that [React compiler](https://react.dev/learn/react-compiler) is also available in LiveCodes and is [documented here](./react.mdx).
...
[TypeScript compiler](./typescript.mdx)
...
[Custom settings](../advanced/custom-settings.mdx) added to the property `jsx` are passed to the TypeScript compiler as [compiler options](https://www.typescriptlang.org/tsconfig#compilerOptions) while compiling JSX.
