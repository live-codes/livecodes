...
The rendered diagrams are added to the [result page](../features/result.mdx) as either:
...
[Pintora config](https://pintorajs.vercel.app/docs/configuration/config) object can be specified in [custom settings](../advanced/custom-settings.mdx), under the key `pintora`.
