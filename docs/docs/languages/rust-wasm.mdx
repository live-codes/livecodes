...
- `livecodes.rust.run`: A function that runs the Rust code again, returning a promise that resolves with an object containing the `output`, `error`, and `exitCode` properties.
...
Miri, the Rust mid-level IR interpreter, compiled to WebAssembly. It is loaded and run inside a Web Worker in the result page, and reports compiler diagnostics, program output and exit codes.
