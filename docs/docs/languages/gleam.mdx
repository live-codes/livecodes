...
LiveCodes compiles Gleam code to JavaScript using the WebAssembly (wasm) version of the [official Gleam compiler](https://github.com/gleam-lang/gleam). The compiled JavaScript code is then executed in the context of the [result page](../features/result.mdx).

The compiled JavaScript code can be inspected in the [Compiled Code Viewer](../features/compiled-code.mdx) in the [Tools Pane](../features/tools-pane.mdx) (below the result page). Console output is shown in the [integrated console](../features/console.mdx).
...
Built modules can then be declared in [custom settings](../advanced/custom-settings.mdx) (Project menu → Custom Settings), under the `gleam` property, by adding a `modules` property.
...
This can then be [mapped (using import maps)](../features/module-resolution.mdx#custom-module-resolution) in [custom settings](../advanced/custom-settings.mdx) (Project menu → Custom Settings) to the full URL of the compiled JavaScript code.
...
[External functions](https://tour.gleam.run/advanced-features/externals/) written in JavaScript can also be used. An external function has the `@external` attribute on it. It needs to specify a "relative" URL specifying the location of the external code. This URL is [mapped (using import maps)](../features/module-resolution.mdx#custom-module-resolution) in [custom settings](../advanced/custom-settings.mdx) (Project menu → Custom Settings) to the full URL of the script that contains the code.
...
[Data URLs](https://developer.mozilla.org/en-US/docs/Web/HTTP/Basics_of_HTTP/Data_URLs) can be used to avoid having to host the external code online. LiveCodes enables [creating data URLs](../features/data-urls.mdx) easily.
...
See list of supported CDNs and the respective modifiers in the section about [module resolution](../features/module-resolution.mdx#cdn-providers).
