...
- `livecodes.haskellWasm.run`: A method that runs the current editor code again and returns a
  promise that resolves to `{ output, error, exitCode }`. Calls are queued, because GHC's
  interactive session cannot execute concurrently.
