...
Please note that using React compiler with TypeScript (TSX) is also supported in LiveCodes and is [documented here](./react-tsx.mdx).

Also note that LiveCodes supports running [JSX](./jsx.mdx) and [TSX](./tsx.mdx) which are compiled to JavaScript using the [TypeScript compiler](./typescript.mdx). This was the traditional way of running React and other JSX/TSX apps in LiveCodes before React compiler support was added.
...
npm modules can be imported as described in the section about [module resolution](../features/module-resolution.mdx), including bare module imports and importing from different CDNs. Stylesheet imports are added as `<link rel="stylesheet">` tags in the page `head`.
...
Types for imported modules are loaded automatically (if available) to provide [Intellisense](../features/intellisense.mdx), auto-completion and type information.
...
Moreover, you can provide custom type definitions for modules that do not have types available on npm. See [Custom Types](../features/intellisense.mdx#custom-types) for details.
...
This can be useful, for example, when using [MDX](./mdx.mdx) to import components exported form JSX.
...
Styles added in the style editor is applied globally to the [result page](../features/result.mdx). This can use different **languages/processors** supported in LiveCodes including CSS, SCSS, Less, Stylus, ..etc. See [style documentation](../features/css.mdx) for more details.
...
[CSS Frameworks](../features/css.mdx#css-processors) supported in LiveCodes (e.g. [Tailwind CSS](./tailwindcss.mdx), [UnoCSS](./unocss.mdx), [WindiCSS](./windicss.mdx)) can detect class names added in JSX. Make sure that the required utility is enabled (from style editor menu or in [`processors`](../configuration/configuration-object.mdx#processors) property of [configuration object](../configuration/configuration-object.mdx)) and that required [directives](https://tailwindcss.com/docs/functions-and-directives#tailwind) are added to the style editor.
...
[Custom settings](../advanced/custom-settings.mdx) can be used to add configuration under the following keys:
