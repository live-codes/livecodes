...
TypeScript is also supported in React Native (TSX) and is [documented here](./react-native-tsx.mdx).
...
For usage and examples, see documentation for [JSX](./jsx.mdx).
...
[Custom settings](../advanced/custom-settings.mdx) added to the property `react-native` are passed to the TypeScript compiler as [compiler options](https://www.typescriptlang.org/tsconfig#compilerOptions) while compiling JSX.
