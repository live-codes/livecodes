...
The included [GitHub Actions workflow](./deployment.mdx#deploy-to-php) deploys on push to the `main` branch (and can be triggered manually). The workflow only runs when the repository variable `PHP_ENV` is set (**Settings → Secrets and variables → Actions → Variables**).
...
See [Deployment](./deployment.mdx) for details about this workflow and the other included deploy workflows.
