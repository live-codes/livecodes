...
**source badge** (e.g. `script:7`, `markup:16`, `index.html:10`, or `src/main.js:2`) that, when
...
- Inline module scripts that begin with `import`/`export` do not get the prologue (prepending
  before a top-level `import` would be a syntax error); their top-level logs resolve to the
  script content-start line.
