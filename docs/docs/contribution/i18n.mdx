...
Other directories under `src/livecodes/i18n/locales` are used to store the translated texts in other target languages. Each of them contains `.ts` files (used by the app) and generated `.lokalise.json` files (used for manually authoring translations on Lokalise).
...
- [`i18n-export`](../../../scripts/i18n-export.js): Extracts source texts from the codebase and generates `.ts` and `.lokalise.json` files under `src/livecodes/i18n/locales/en`.
  - The `--save-temp` flag can be used to save the extracted source texts to `i18n/locales/tmp` instead of `en` for debugging purposes.
...
- [`i18n-update-pull` / `i18n-import.mjs`](../../../scripts/i18n-import.mjs): **(Only used in CI and should not be run locally)** Pulls the translated texts from Lokalise and updates the `.ts` files under `src/livecodes/i18n/locales`. _Outdated translation will be deprecated during import._ Keys that are missing from the Lokalise branch fall back to the existing translation in the repository, so translations are not lost when Lokalise is behind the codebase.
