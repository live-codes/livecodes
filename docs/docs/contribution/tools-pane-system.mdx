...
Console entries can carry a source badge (file + line) that navigates the editor on click; the
mapping pipeline is documented in [Source Maps & Console Line Numbers](./sourcemaps-console-line-numbers.mdx).
