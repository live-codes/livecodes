...
- Additional query parameters include:
  ...
  - `activityId`: `string`.
    ...
    An id used to key this project's autosaved state in the browser's local storage, independent of the normal randomly-generated project id. On load, if a saved project already exists for this `activityId`, it is loaded instead of any starter code supplied via `config` or other query parameters. Not part of the [configuration object](./configuration-object.mdx) — it is not included in exported projects or generated share URLs.
