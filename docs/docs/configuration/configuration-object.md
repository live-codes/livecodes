...
The configuration object can be used while initializing a new playground using the [library API](../advanced/api.md).
...
This is a read-only property which specifies the current LiveCodes version. It can be shown using the API [`exec`](../advanced/api.md#exec) method.
