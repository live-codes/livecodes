...
function JS(st) {
  ...
  function JSEVAL(ast, cparams) {
    ...
    console.log(pdecl);
    pdstr = jsstr.val.replace('${}', `Module.cwrap('${pdecl.name}', 'string', [])()`) + '\\00';
  ...
  function SETTIMEOUT(ast, cparams) {
    var ir = [],
      pptr = cparams[0],
      interval = cparams[1],
      pdecl = st.lookup(pptr.id),
    ...
    // TODO: check that interval is INTEGER literal
    ir.push('  ; SETINTERVAL start');
    pdstr = "setTimeout(Module.cwrap('" + pdecl.name + "', 'void', []), " + interval.val + ')\\00';
    pdlen = pdstr.length - 2; // for the escaped null terminator
    pditype = '[' + pdlen + ' x i8]';
    ir.push([pstr + ' = private constant ' + pditype + ' c"' + pdstr + '"']);
    ir.push('  ' + str + ' = getelementptr inbounds ' + pditype + '* ' + pstr + ', i32 0, i32 0');
  ...
  pins('JS', JS, [{ type: { node: 'type', name: 'STRING' } }]);
  pins('JSEVAL', JSEVAL, [
