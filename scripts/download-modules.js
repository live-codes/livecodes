...
const downloadModules = async ({ dryRun = false } = {}) => {
  ...
  // download modules
  const download = (
    /** @type {Array<{module: string; url: string}>} */ moduleUrls,
    isRetry = false,
  ) => {
    for (const { module, url } of moduleUrls) {
      ...
      if (dryRun) {
        ...
        downloadQueue.add(() =>
          fetchAndSaveFile(url, fullPath).then(async (result) => {
...
const createAsyncQueue = (concurrency = 1) => {
  /** @typedef {(() => Promise<unknown> | unknown) | Promise<unknown>} Task */
  ...
  const runSubscribers = async () => {
    if (queue.length === 0 && running === 0 && subscribers.length > 0) {
  ...
  const onFinish = (/** @type {() => unknown} */ cb) => {
    subscribers.push(cb);
  };
