...
const translation: I18nTranslation = {
  ...
  core: {
    ...
    error: {
      ...
      failedToCopyImage: 'イメージのコピーに失敗しました',
  ...
  import: {
    ...
    code: {
      desc: 'サポートされているソース：<1> <2>GitHub gist</2> <3>GitHubファイル</3> <4>GitHubリポジトリ内のディレクトリ</4> <5>Gitlab snippet</5> <6>Gitlabファイル</6> <7>Gitlabリポジトリ内のディレクトリ</7> <8>JS Bin</8> <9>生のコード</9> <10>Webページ内のDOM内のコード</10> <11>ZIPファイル内のコード</11> <12>画像内のコード（OCR）</12> <13>公式プレイグラウンド<14></14>（TypeScript、Vue）</13> </1> 詳細については<15>ドキュメント</15>をご覧ください。',
