// from https://github.com/microsoft/TypeScript-Website/blob/v2/packages/sandbox/src/twoslashSupport.ts
    ...
    case 'list':
      ...
      if (typeof elementType === 'string') {
        ...
        opts[opt.name] = strings
          .map((v) => getOptionValueFromMap(opt.name, v, elementType as Map<string, string>)!)
          .filter(Boolean);
      }
      break;
...
export const twoslashCompletions =
  ...
  (
    ...
    const result: Array<import('monaco-editor').languages.CompletionItem> = [];
