 ...
 */
export function replaceWithSnippet(
...
export function toRange(
  ...
  return [editor.posFromIndex(range[0]), editor.posFromIndex(range[1])];
}
...
export function attributeValue(attr: AttributeToken): string | undefined {
  const { value } = attr;
  return value && isQuoted(value) ? value.slice(1, -1) : value;
}
...
export function textRange(editor: CodeMirror.Editor, range: CMRange): TextRange {
  ...
  return [Math.min(head, anchor), Math.max(head, anchor)];
}
 ...
 */
export function getSelectionsFromSnippet(
  snippet: string,
  base = 0,
): { ranges: TextRange[]; snippet: string } {
