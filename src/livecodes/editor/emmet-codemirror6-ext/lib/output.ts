...
export function field() {
  ...
  return (index: number, placeholder: string) => {
    if (handled === -1 || handled === index - 1) {
      handled = index;
      return placeholder ? tabStopStart + placeholder + tabStopEnd : tabStopStart;
    }
 ...
 */
export function lineIndent(editor, line: number): string {
 ...
 */
export function getIndentation(editor): string {
